        ...
        <div class="jumbotron">
            ...
            <table class="table table-stripped table-bordered">
                ...
                <tbody>
                    ...
                    <tr>
                        <th><img src="{{ asset('uploads/product/' . $product->image )}}" width="100px;" height="100px;" alt="Image"></th>
                        <th>{{$product->productname}}</th>
                        <th>{{$product->productdesc}}</th>
                        <th>{{$product->stock}} Items</th>
                        <th>RM {{$product->price}}</th>
                        <form action="{{ url('addtocart')}}/{{ $product->product_id }}" method="post">
                        @csrf
                            <th><input type="number" name="quantity" class="form-control" required="" autocomplete="off"></th>
                            <th><button type="submit" class="btn btn-primary">Add to cart</button></th>
                        </form>
                    </tr>
                    @endforeach
                </tbody>
